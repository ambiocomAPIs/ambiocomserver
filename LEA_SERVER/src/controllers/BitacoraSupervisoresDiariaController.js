  ...
  try {
    ...
    if (!fecha || !turno) {
      return res.status(400).json({
        message:
          "Se requieren 'fecha' y 'turno' como parámetros.",
      });
    }

    const result = await Bitacora.find({
      fecha,
      turno,
    });
    ...
    if (result.length === 0) {
      return res.status(404).json({
        message:
          "No se encontraron datos para la fecha y turno especificados.",
      });
    }
    ...
    console.error(
      "❌ Error al obtener datos por fecha y turno:",
      error
    );
...
export const bitacorareplaceall = async (req, res) => {
  try {
    if (
      ...
      return res.status(400).json({
        message:
          "Se requiere un arreglo con al menos un objeto.",
      });
    }
    ...
    if (!fecha || !turno) {
      return res.status(400).json({
        message:
          "Faltan 'fecha' o 'turno' en la solicitud.",
      });
    }
    ...
    console.error(
      "❌ Error en replaceAllBitacora:",
      error
    );

    res.status(500).json({
      message:
        "Error al guardar los datos de bitácora.",
    });
  }
};
...
export const updateDataById = async (req, res) => {
  try {
    const updated = await Bitacora.findByIdAndUpdate(
      req.params.id,
      {
        $set: req.body,
      },
    ...
    if (!updated) {
      return res.status(404).json({
        message:
          "Documento no encontrado para actualizar",
      });
    }
