import MedidoresAgua from "../models/MedidoresAguaModel.js";
...
export const crearMedicionAgua = async (req, res) => {
  try {
    ...
    if (!fecha || !hora || !operador || !lecturas) {
    ...
    res.status(500).json({
      message: "Error al crear medición de agua",
      error: error.message,
    });
  }
};
...
export const obtenerMedicionesAgua = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Error al obtener mediciones de agua",
      error: error.message,
    });
  }
};
  ...
  try {
    ...
    if (!medicion) {
      return res.status(404).json({
        message: "Medición de agua no encontrada",
      });
    }
    ...
    res.status(500).json({
      message: "Error al obtener medición de agua",
      error: error.message,
    });
  }
};
